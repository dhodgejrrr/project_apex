...
class RaceReportGenerator:
    """
    Processes race timing data with a fully dynamic handicap system that identifies
    the top-performing license class as the baseline for all adjustments.
    """
            ...
            for lap in all_laps_for_car:
                ...
                if lap.get('is_valid') and 'sector_times' in lap and len(lap['sector_times']) >= 3:
                    ...
                    if all(t is not None for t in [lap_time, s1_time, s2_time, s3_time]):
                        flat_laps.append({
                            'driver_name': driver_map[driver_num]['full_name'], 'license': driver_map[driver_num]['license'],
                            'vehicle': participant.get('vehicle'), 'class': participant.get('class'),
                            'lap_time': lap_time, 's1_time': s1_time, 's2_time': s2_time, 's3_time': s3_time,
    ...
    def _calculate_stats(df_slice, time_column):
        ...
        best_3_laps, best_5_laps = df.head(3), df.head(5)
        return {
            'fastest': fastest_row[time_column], 'fastest_stint_lap': int(fastest_row['stint_lap_number']),
    ...
    def _calculate_all_driver_stats(self, laps_df):
        ...
        for group_keys, group_df in laps_df.groupby(['driver_name', 'license', 'vehicle', 'class']):
            self.driver_stats.append({
                'driver_name': group_keys[0], 'license': group_keys[1], 'vehicle': group_keys[2], 'class': group_keys[3],
    ...
    def _calculate_dynamic_license_factors(self):
        """
        Calculates license factors using a fully dynamic baseline determined by the
        fastest overall license class in the session.
        """
        ...
        for license_class in valid_licenses:
            ...
            if license_class == 'Bronze':
                ...
                if leaders.empty: class_paces['Bronze'] = pack['pace'].mean() if not pack.empty else None
                elif pack.empty: class_paces['Bronze'] = leaders['pace'].mean()
                else: class_paces['Bronze'] = (leaders['pace'].mean() * self.PACE_LEADER_WEIGHT) + (pack['pace'].mean() * (1 - self.PACE_LEADER_WEIGHT))
            else:
                cutoff = class_df['pace'].quantile(self.COMPETITIVE_PACE_PERCENTILE)
                competitive_group = class_df[class_df['pace'] <= cutoff]
                if not competitive_group.empty: class_paces[license_class] = competitive_group['pace'].mean()

        if not class_paces:
            print("Warning: Could not establish any baseline pace. Aborting handicap calculation.")
            return

        # --- DYNAMIC BASELINE IDENTIFICATION ---
        baseline_class = min(class_paces, key=class_paces.get)
        baseline_pace = class_paces[baseline_class]
        self.baseline_license_class_used = baseline_class
        print(f"\nIdentified '{baseline_class}' as the performance baseline for this session (Pace: {baseline_pace:.3f}s).")
    ...
    def _add_stint_adjusted_scores(self):
        ...
        for driver in self.driver_stats:
            for time_key in ['lap_times', 'sector_1_times', 'sector_2_times', 'sector_3_times']:
                ...
                stats['stint_adjusted_best_3_avg'] = stats.get('best_3_avg', 0) + (stats.get('avg_stint_lap_for_best_3', 0) * self.STINT_LAP_PENALTY)
                stats['stint_adjusted_best_5_avg'] = stats.get('best_5_avg', 0) + (stats.get('avg_stint_lap_for_best_5', 0) * self.STINT_LAP_PENALTY)
    ...
    def _add_license_adjusted_scores(self):
        ...
        if not self.dynamic_license_factors: return
        for driver in self.driver_stats:
            ...
            for time_key in ['lap_times', 'sector_1_times', 'sector_2_times', 'sector_3_times']:
                ...
                stats['license_adjusted_best_3_avg'] = stats.get('best_3_avg', 0) * factor
                stats['license_adjusted_best_5_avg'] = stats.get('best_5_avg', 0) * factor
    ...
    def _add_combined_adjusted_scores(self):
        ...
        for driver in self.driver_stats:
            ...
            for time_key in ['lap_times', 'sector_1_times', 'sector_2_times', 'sector_3_times']:
                ...
                stats['combo_adjusted_best_3_avg'] = stats.get('stint_adjusted_best_3_avg', 0) * factor
                stats['combo_adjusted_best_5_avg'] = stats.get('stint_adjusted_best_5_avg', 0) * factor
    ...
    def _generate_rankings(self):
        ...
        def create_ranking(stats, time_key, metric_key):
            ranked_list = [{'driver_name': d['driver_name'], 'license': d['license'], 'vehicle': d['vehicle'], 'value': d.get(time_key, {}).get(metric_key)}
        ...
        def generate_rankings_for_group(stats):
            ...
            for metric in metrics_to_rank:
                for name, key in time_metrics.items(): rankings[f'by_{metric}_{name}'] = create_ranking(stats, key, metric)
            return rankings
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not participants: raise ValueError("No participants found in the data file.")
        ...
        for race_class in unique_classes:
            ...
            output_filename = f"race_report_v2_{race_class}.json"
